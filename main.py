...
# Підключення до Qdrant локально або в хмарі
client = QdrantClient(url="http://localhost:6333")  # Якщо у вас запущений локальний сервер
...
def store_chunks_in_qdrant(chunks, model_name="all-MiniLM-L6-v2"):
    try:
...
def get_gemini_response(context_chunks, user_query):
    try:
        # Об'єднуємо всі релевантні чанки в контекст
        context = "\n".join(context_chunks)

        # Ініціалізація LLM (chat-based interaction)
        ...
        # Виконуємо запит до LLM на основі сформованих повідомлень
        prompt = ChatPromptTemplate.from_messages(messages)

        # Ланцюжок обробки з LLM
...
def main(file_path, user_query):
    ...
    # Зберігаємо чанки в Qdrant
    if not store_chunks_in_qdrant(chunks):
        print("Не вдалося створити Qdrant колекцію.")
        return
    ...
    # Генерація відповіді за допомогою Gemini
